...
const  conn = mysql.createConnection({
  ...
  dataPool.allNovice=()=>{
    return new Promise ((resolve, reject)=>{                        //resolve in reject sprejme
      conn.query(`SELECT * FROM news`, (err,res)=>{                 //callback error ce ni uredu
        if(err){return reject(err)}                                 //rejectaj error
        return resolve(res)
      })
    })
  }
  ...
  dataPool.oneNovica=(id)=>{
    return new Promise ((resolve, reject)=>{
      conn.query(`SELECT * FROM news WHERE id = ?`, id, (err,res)=>{
        ...
        return resolve(res)                                       //if we succeed
      })
    })
  }
  ...
  dataPool.createNovica=(title,slug,text)=>{
    return new Promise ((resolve, reject)=>{
      conn.query(`INSERT INTO news (title,slug,text) VALUES (?,?,?)`, [title, slug, text], (err,res)=>{ //into table news
        if(err){return reject(err)}
        return resolve(res)
      })
    })
